...
2. On the command line prompt inside the container, verify that you **_cannot_** read the content of the file containing the password:

   ```bash
   cat /etc/api_password.txt
   ``` 

1. To make sure the file with the password is not visible somewhere else inside the container we also try and find a file with that name somewhere else on the filesystem:

   ```bash
   find / -name "*api_password*"
   ```

1. In a last attempt to find the password, we look through the generated files in the repository, by executing the following command locally, i.e. outside of the container:

   ```bash
   find ./target -name "*api_password*"
   ```
   
We notice that all our attempts to find the value of the password fail, since the password is confidentiality protected, also against somebody with root access rights to the system of the application owner.  
