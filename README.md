...
> **NOTE:** If you want to learn how to verify that the desired levels of confidentiality, integrity and consistency protection are achieved,
> you can jump straight to [**Verify the Desired Protection Level**](#verify-the-desired-protection-level).
...
> application. All other changes are automatically detected and cause the
...
to which you want to deploy your Hello World application.
  ...
  > **NOTE:** Assuming you have a kubeconfig file, you can get access to the cluster by:
  > 1. putting your kubeconfig file at `~/.kube/config`, or 
  > 3. setting your `$KUBECONFIG` environment variable to point to it, or
  > 4. adding the option `--kubeconfig /path/to/your/kubeconfig.file` to each of the following `kubectl` commands.
...
We read the environment variables periodically to be able to show below that these variables are properly protected.
In the program we also check whether any of the environment variables has changed, print an error and exit if they did.
This will however never happen, since the SCONE runtime system will detect the changes for us and not execute the program.
...
Not all secrets might be suitable to declare as an environment variable, as we did in [the example with `API_PASSWORD`](#quick-start-guide).
Many times the sensitive data might be provided to the service through a file.
In this section we will show how to read the password from a file instead of from an environment variable, and still ensure its confidentiality, integrity and consistency protection.

> **NOTE:** Before you start, make sure you [install the prerequisites](#step-0-requirements).

### Step 1: Rewriting the python program

In this sample program we want to read the password from file instead of from an environment variable.
We therefore change the following lines in the `print_env.py` file:
...
# The value of API_PASSWORD is generated by CAS and therefore 
# not visible even to the sysadmin of the application owner.
with open('/etc/api_password.txt') as pw_file:
    pw = pw_file.read().strip()  # we assume the password is the only file content
...
```
...
...
    with open('/etc/api_password.txt') as pw_file:
        new_pw = pw_file.read().strip()
...
```

Although not strictly necessary, we also want to change the comments and the statements that are printed out, to avoid confusion when examining the log of the application. 
Therefore, change all other occurrences of '`API_PASSWORD`', e.g., to 'api password', as well.


### Step 2: Adjust the Manifest files

We now need to declare that our service expects the password as a file instead of as an environment variable, so that we inject it into the enclave in the correct manner.

1. We start by removing the declaration of `API_PASSWORD` as an environment variables in `service.yaml`.
This is done by removing the following lines:
   ...
   from the field `environment.local`.
1. Then we declare that it should be injected into the file `/etc/api_password.txt` in the enclave instead.
To do so we add the following lines at the top level in `service.yaml`:

   ```bash
   ...
   injection_files:
     - path: /etc/api_password.txt
       content: $$SCONE::password$$  # get from CAS
   ...
   ```

### Step 3: Build and Deploy Your New Application

Now all that is left is to build and deploy your new application. Please [run `./run.sh`](#step-3-build-and-deploy-your-application) to do so.
Use `kubectl logs <pod name>` to check the log of the application. It should look something like:
...
musl version: 1.1.24
SCONE version: 5.7.0 (Tue Jan 18 08:45:24 2022 +0100) 
Enclave hash: fe0d9a2e83a7fc5a3b68919634a501ed6e3518bfb7fc235a8ad72fb3f0ff9a5c
Application runs in SGX production mode
Hello, 'myself' - thanks for passing along the api password
The checksum of the original api password is 'aa9179afd66fde7a3711ed2adda02f6b'
Hello, user 'myself'!
The checksum of the current password is 'aa9179afd66fde7a3711ed2adda02f6b'
Stop me by executing 'helm uninstall pythonapp'
...
```

If you want you can also verify the confidentiality, integrity and consistency protection of your password. 
We explain how in the next section.

## Verify the Desired Protection Level

It is an important objective of our application that the password is [confidentiality, integrity as well as consistency protected](#objectives).
Neither the system administrator of the applcation owner nor anybody else should be able to read, modify or revert the password to an older version.
We will now make sure these objectives are met.

> **NOTE:** To check the protection of the password,
> we will need to examine the logs of the container running
> the `Hello World` service,
> and/or execute some commands inside the container itself.
> To perform those actions, you can use the following commands:
...
One of our objectives in this tutorial, is to ensure the confidentiality of our password stored in the file `/etc/api_password.txt` in the container.

We perform the follwoing steps to verify that we cannot read the password:
...
2. First we look for the file `/etc/api_password.txt` but we soon see it is not present at the expected location in the container:

   ```bash
   ls /etc/api_password.txt
   ``` 

3. To make sure the file with the password is not present somewhere else inside the container, we execute the following `find` command, which fails to find anything:

   ```bash
   find / -name "*api_password*"
   ```

4. In a final attempt to find the file with the password, we look through the generated files in the repository. 
We do this by executing the following command locally, i.e. outside of the container:

   ```bash
   find target -name "*api_password*"
   ```
   
We notice that all our attempts to find the value of the password fail, since the password is confidentiality protected.
In fact, not even somebody with root access rights to the system of the application owner, can access it.  


### Verify Integrity Protection

Another of our objectives, is to ensure the integrity of our password, that is stored in the file `/etc/api_password.txt` inside of the container.
To verify that the desired level of integrity protection is achieved, we therefore have to verify that one cannot change the password without the change being detected.

To check the integrity of the password, we perform the following steps:


1. Look in the log of the container and remember the hash of the current password:
...
2. Write a new password to the file `/etc/api_password.txt` inside the container:

   ```bash
   echo "bad password" > /etc/api_password.txt 
   ```

1. Log out and look in the logs of the container again, to verify that the hash of the password has not changed:

   ```bash
   kubectl logs <pod name>
   ```

Although we succeed to create the file with new content
when examining the logs
we see that the hash of the password
is identical to the one we remember from the previous step.


### Verify Consistency Protection

The third of our objectives in this tutorial, is to ensure the consistency of our password. To verifiy that this objective is achieved, we therefore need to show that, as soon as the process of the service starts up, we detect if somebody tried to revert to an older version of the password.

> **NOTE:** Changing the values of the password
> in any other way than running an unintended, previously valid version,
> is prevented by the integrity protection, as opposed to the consistency
> protection.

To simulate an attack on the consistency protection of the password, we first deploy the application with a new password, and then maliciously try and deploy the application with the old version of the password.

To verify the consistency protection of the password, we would first have to create and deploy a new version of the application, i.e., version 2, which only differs from version 1 in its password.
...
However, as soon as the password is regenerated to be used in version 2, the old password is no longer present, neither in CAS, nor anywhere else, and we therefore have nothing to which we can revert. 

But for the sake of argument, let us assume that the attacker in some way had indeed gotten a hold of a copy of the version 1 password. 
The attacker would have to upload it to CAS, in order for it to become the correct password. As soon as the upload to CAS has taken place, however,
this is considered to be a new, authorized version, i.e., version 3.
